...
func TestComputeBlockLayoutInfo(t *testing.T) {
	tests := []struct {
		name            string
		rootNode        any
		leafOrder       []remotetermobj.LeafOrderEntry
	...
	}{
		{
			...
			leafOrder: []remotetermobj.LeafOrderEntry{{NodeId: "n1", BlockId: "b1"}},
			wantGeometry: map[string]geomWant{
				"b1": {0, 0, 1, 1},
			},
		...
		},
		{
			...
			leafOrder:     []remotetermobj.LeafOrderEntry{{NodeId: "n1", BlockId: "b1"}, {NodeId: "n2", BlockId: "b2"}},
			focusedNodeId: "n1",
			wantGeometry: map[string]geomWant{
				"b1": {0, 0, 0.5, 1},
				"b2": {0.5, 0, 0.5, 1},
