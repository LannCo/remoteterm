...
func EnsureInitialData() (bool, error) {
	...
	rtstore.SetClientId(client.OID)
...
func CreateClient(ctx context.Context) (*remotetermobj.Client, error) {
	client := &remotetermobj.Client{
		OID:       uuid.NewString(),
		WindowIds: []string{},
	}
	err := rtstore.DBInsert(ctx, client)
	if err != nil {
...
func GetClientData(ctx context.Context) (*remotetermobj.Client, error) {
...
func SendWaveObjUpdate(oref remotetermobj.ORef) {
	ctx, cancelFn := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancelFn()
	// send a waveobj:update event
	waveObj, err := rtstore.DBGetORef(ctx, oref)
	if err != nil {
	...
	wps.Broker.Publish(wps.WaveEvent{
		...
		Data: remotetermobj.WaveObjUpdate{
			UpdateType: remotetermobj.UpdateType_Update,
			OType:      waveObj.GetOType(),
			OID:        remotetermobj.GetOID(waveObj),
			Obj:        waveObj,
		},
	})
}
...
func InitMainServer() error {
	...
	mainServer, err := rtstore.DBGetSingleton[*remotetermobj.MainServer](ctx)
	if err == rtstore.ErrNotFound {
		mainServer = &remotetermobj.MainServer{
			OID: uuid.NewString(),
		}
		err = rtstore.DBInsert(ctx, mainServer)
		if err != nil {
	...
	if needsUpdate {
		err = rtstore.DBUpdate(ctx, mainServer)
		if err != nil {
	...
	err = remotetermjwt.SetPrivateKey(privateKeyBytes)
	if err != nil {
