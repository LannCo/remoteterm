...
func SwitchWorkspace(ctx context.Context, windowId string, workspaceId string) (*remotetermobj.Workspace, error) {
	...
	allWindows, err := rtstore.DBGetAllObjsByType[*remotetermobj.Window](ctx, remotetermobj.OType_Window)
	if err != nil {
	...
	err = rtstore.DBUpdate(ctx, window)
	if err != nil {
...
func GetWindow(ctx context.Context, windowId string) (*remotetermobj.Window, error) {
...
func CreateWindow(ctx context.Context, winSize *remotetermobj.WinSize, workspaceId string) (*remotetermobj.Window, error) {
	log.Printf("CreateWindow %v %v\n", winSize, workspaceId)
	var ws *remotetermobj.Workspace
	if workspaceId == "" {
	...
	if winSize == nil {
		winSize = &remotetermobj.WinSize{
			Width:  0,
			Height: 0,
		}
	}
	window := &remotetermobj.Window{
		OID:         windowId,
		WorkspaceId: ws.OID,
		IsNew:       true,
		Pos: remotetermobj.Point{
			X: 0,
			Y: 0,
		},
		WinSize: *winSize,
	}
	err := rtstore.DBInsert(ctx, window)
	if err != nil {
	...
	err = rtstore.DBUpdate(ctx, client)
	if err != nil {
...
func CloseWindow(ctx context.Context, windowId string, fromElectron bool) error {
	...
	if err == nil {
		...
		err = rtstore.DBDelete(ctx, remotetermobj.OType_Window, windowId)
		if err != nil {
...
func CheckAndFixWindow(ctx context.Context, windowId string) *remotetermobj.Window {
...
func FocusWindow(ctx context.Context, windowId string) error {
	...
	return rtstore.DBUpdate(ctx, client)
}
