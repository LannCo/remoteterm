...
wsh token "$REMOTETERM_SWAPTOKEN" fish 2>/dev/null | source
set -e REMOTETERM_SWAPTOKEN

# Load Wave completions
wsh completion fish | source

set -g _REMOTETERM_SI_FIRSTPROMPT 1
...
function _waveterm_si_prompt --on-event fish_prompt
    ...
    if test $_REMOTETERM_SI_FIRSTPROMPT -eq 1
    ...
    set -g _REMOTETERM_SI_FIRSTPROMPT 0
end
