...
func (client *Client) Connect() error {
	jwtToken := remotetermbase.GetEnvNewOrLegacy(wshutil.WaveJwtTokenVarName, wshutil.LegacyWaveJwtTokenVarName)
	if jwtToken == "" {
...
func determineMimeType(option FileHandlerOption) (string, []byte) {
	...
	if option.FilePath != "" {
		filePath := remotetermbase.ExpandHomeDirSafe(option.FilePath)
