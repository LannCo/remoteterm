...
func makeSwapToken(ctx context.Context, logCtx context.Context, blockId string, blockMeta waveobj.MetaMapType, remoteName string, shellType string) *shellutil.TokenSwapEntry {
	...
	wavebase.SetDualEnv(token.Env, wavebase.WaveBlockIdVarName, wavebase.LegacyWaveBlockIdVarName, blockId)
	token.Env[wavebase.WaveVersionVarName] = wavebase.WaveVersion
	wavebase.SetDualEnv(token.Env, wavebase.WaveFlagVarName, wavebase.LegacyWaveFlagVarName, "1")
	tabId, err := wstore.DBFindTabForBlockId(ctx, blockId)
	if err != nil {
		log.Printf("error finding tab for block: %v\n", err)
	} else {
		wavebase.SetDualEnv(token.Env, wavebase.WaveTabIdVarName, wavebase.LegacyWaveTabIdVarName, tabId)
	}
	if tabId != "" {
		...
		if err != nil {
			...
			wavebase.SetDualEnv(token.Env, wavebase.WaveWorkspaceIdVarName, wavebase.LegacyWaveWorkspaceIdVarName, wsId)
		}
	}
	wavebase.SetDualEnv(token.Env, wavebase.WaveClientIdVarName, wavebase.LegacyWaveClientIdVarName, wstore.GetClientId())
	wavebase.SetDualEnv(token.Env, wavebase.WaveConnVarName, wavebase.LegacyWaveConnVarName, remoteName)
