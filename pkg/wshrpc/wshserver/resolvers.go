...
func resolveTitle(ctx context.Context, data wshrpc.CommandResolveIdsData, value string) (*remotetermobj.ORef, error) {
	if data.BlockId == "" {
		return nil, fmt.Errorf("no blockid in request")
	}
	tabId, err := rtstore.DBFindTabForBlockId(ctx, data.BlockId)
	...
	for _, blockId := range tab.BlockIds {
		...
		titles[blockId] = block.Meta.GetString(remotetermobj.MetaKey_FrameTitle, "")
	}
	matches, err := matchBlockTitles(titles, value)
	if err != nil {
		return nil, err
	}
	return &remotetermobj.ORef{OType: remotetermobj.OType_Block, OID: matches[0]}, nil
}
