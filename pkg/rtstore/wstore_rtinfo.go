...
var (
	rtInfoStore = make(map[remotetermobj.ORef]*remotetermobj.ObjRTInfo)
...
func SetRTInfo(oref remotetermobj.ORef, info map[string]any) {
	rtInfoMutex.Lock()
	defer rtInfoMutex.Unlock()

	rtInfo, exists := rtInfoStore[oref]
	if !exists {
		rtInfo = &remotetermobj.ObjRTInfo{}
...
func GetRTInfo(oref remotetermobj.ORef) *remotetermobj.ObjRTInfo {
	rtInfoMutex.RLock()
...
func DeleteRTInfo(oref remotetermobj.ORef) {
	rtInfoMutex.Lock()
	defer rtInfoMutex.Unlock()
