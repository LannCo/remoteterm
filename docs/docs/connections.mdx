...
If this fails for some reason, RemoteTerm will attempt to run without `wsh`. You will see this indicated by a small **<code><i className="fa-link-slash fa-solid fa-sharp"/></code>** icon in the block header. For more info on what `wsh` is capable of, see [wsh command](/wsh). And if you wish to view the source code of `wsh`, you can find it [here](https://github.com/LannCo/remoteterm/tree/main/cmd/wsh).
...
| conn:keepaliveinterval | This int (seconds) controls how long the connection can sit idle before the monitor sends a keepalive probe. It defaults to `3`. See [Connection Resilience](/reconnect) for details. |
| conn:stallthreshold | This int (seconds) controls how long the monitor waits for a keepalive response before declaring the connection stalled. It defaults to `3`. See [Connection Resilience](/reconnect) for details. |
| conn:reconnecttimeout | This int (seconds) controls how long each reconnect attempt waits for the SSH handshake before giving up. It defaults to `5`. See [Connection Resilience](/reconnect) for details. |
| conn:reconnectinterval | This int (seconds) controls how long the reconnect scheduler waits between attempts in normal mode. It defaults to `5`. See [Connection Resilience](/reconnect) for details. |
| conn:reconnectaggressiveinterval | This int (seconds) controls how long the reconnect scheduler waits between attempts in aggressive mode (triggered by network-unreachable errors). It defaults to `3`. See [Connection Resilience](/reconnect) for details. |
