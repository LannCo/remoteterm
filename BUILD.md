...
These instructions are for setting up dependencies and building RemoteTerm from source on macOS, Linux, and Windows.
...
```sh
git clone git@github.com:LannCo/remoteterm.git
```

or

```sh
git clone https://github.com/LannCo/remoteterm.git
```
...
Backend logs for the development version of RemoteTerm can be found at `~/.waveterm-dev/waveapp.log`. Both the NodeJS backend from Electron and the main Go backend will log here.
