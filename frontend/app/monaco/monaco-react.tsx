...
function createModel(value: string, path: string, language?: string) {
    const uri = monaco.Uri.parse(`remoteterm://editor/${encodeURIComponent(path)}`);
...
export function MonacoDiffViewer({ original, modified, language, path, options, onMount }: DiffViewerProps) {
    ...
    useEffect(() => {
        ...
        const origUri = monaco.Uri.parse(`remoteterm://diff/${encodeURIComponent(path)}.orig`);
        const modUri = monaco.Uri.parse(`remoteterm://diff/${encodeURIComponent(path)}.mod`);
