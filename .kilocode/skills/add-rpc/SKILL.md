---
name: add-rpc
description: Guide for adding new RPC calls to RemoteTerm. Use when implementing new RPC commands, adding server-client communication methods, or extending the RPC interface with new functionality.
---
...
RemoteTerm uses a WebSocket-based RPC (Remote Procedure Call) system for communication between different components. The RPC system allows the frontend, backend, electron main process, remote servers, and terminal blocks to communicate with each other through well-defined commands.
...
RPC commands in RemoteTerm follow these conventions:
