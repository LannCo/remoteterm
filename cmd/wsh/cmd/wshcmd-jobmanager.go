...
var jobManagerCmd = &cobra.Command{
	Use:    "jobmanager",
	Hidden: true,
	Short:  "job manager for RemoteTerm",
	Args:   cobra.NoArgs,
	RunE:   jobManagerRun,
}
...
func jobManagerRun(cmd *cobra.Command, args []string) error {
	...
	publicKeyB64 := getEnvNewOrLegacy("REMOTETERM_PUBLICKEY", "WAVETERM_PUBLICKEY")
	if publicKeyB64 == "" {
		return fmt.Errorf("REMOTETERM_PUBLICKEY environment variable is not set")
	}

	publicKeyBytes, err := base64.StdEncoding.DecodeString(publicKeyB64)
	if err != nil {
		return fmt.Errorf("failed to decode REMOTETERM_PUBLICKEY: %v", err)
	}
